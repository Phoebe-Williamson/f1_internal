...
?>
	...
	<body>
		...
		<!-- Button to go back to driver page-->
		<a class="two" href="driver.php">←Back</a>
		<!--Starts grid-->
		<grid>
			<!-- Grid for left side of page where image is displayed-->
			<div class="profile1">
					...
					if(isset($_GET['DriverID'])) {
						...
						// gets driverid from driver page
						$show_driver = "SELECT * FROM Driver WHERE DriverID='".$DriverID."'";

						// query bio and driver table
						...
						while ($row = mysqli_fetch_assoc($result)) {
							echo '<img src="Images/' . $row['Image'] . '" width="700" height="700" alt="' . $row['Image'] . '">'; // displays image of driver
						}
					?>
			</div>	
			<!-- Grid for right side of page where info is displayed-->
			<div class="profile2">
					<?php
					if(isset($_GET['DriverID'])) {
						$DriverID = $_GET['DriverID'];
						...
						while ($row = mysqli_fetch_assoc($result)) { // shows info for driver
							echo '<p>';
